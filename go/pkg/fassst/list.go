...
func List(fs pkgfs.FileSystem, startPath string, routines int, cont func([]pkgfs.FileEntry), log *zap.Logger) {
	...
	runWG.Add(1)
	go lister(fs, startPath, nil, runChan, &runWG, cont, log)
...
func lister(
	...
	cont func([]pkgfs.FileEntry), log *zap.Logger,
) {
	runChan <- utils.U
	defer func() { <-runChan }()
	...
	if err != nil {
		if strings.Contains(err.Error(), "SlowDown") {
			...
			go lister(fs, url, nil, runChan, runWG, cont, log)
			return
		}
	...
	if new_pagination != nil {
		runWG.Add(1)
		go lister(fs, url, new_pagination, runChan, runWG, cont, log)
	}

	for _, d := range dirs {
		runWG.Add(1)
		go lister(fs, pkgfs.MakeSureHasSuffix(d.Name(), "/"), nil, runChan, runWG, cont, log)
	}
